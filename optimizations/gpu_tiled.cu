...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    int shared_size = S * (TILE_WIDTH - 1) + K;
    ...
    #define shared_mem(i2, i1, i0) SharedMem[(i2) * (shared_size * shared_size) + (i1) * shared_size + i0]
    int W_grid = ((W_out - 1) / TILE_WIDTH) + 1;

    int bx = blockIdx.x;
    int by = blockIdx.y;
    int height = (blockIdx.z / W_grid) * TILE_WIDTH + threadIdx.y;
    int width = (blockIdx.z % W_grid) * TILE_WIDTH + threadIdx.x;

    for (int c = 0; c < C; c++){
        for(int j = threadIdx.x; j < shared_size; j += TILE_WIDTH){
            for(int i = threadIdx.y; i < shared_size; i += TILE_WIDTH){
                if ((height - threadIdx.y) * S + i < H && (width - threadIdx.x) * S + j < W)
                    shared_mem(c, i, j) = in_4d(bx, c, (height - threadIdx.y) * S + i, (width - threadIdx.x) * S + j);
                else
                    shared_mem(c, i, j) = 0.0f;    
            }
        }
    }
    __syncthreads();
    if (height < H_out && width < W_out) {
       float out = 0.0f;
        for (int c = 0; c < C; c++) {
            for (int p = 0; p < K; p++) {
                for (int q = 0; q < K; q++) {
                    out += shared_mem(c, threadIdx.y * S + p, threadIdx.x * S + q) * mask_4d(by, c, p, q);
                }
            }
        }
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    int share_width = (TILE_WIDTH - 1) * S + K;
    int dimShared = (C * share_width * share_width * sizeof(float));
